# VS3 Source Tree Snapshot Entry Guard Checkpoint
...
- Harden the VS3 scenario gate so each `source_tree.verified_source_snapshot_paths` entry must match the current source snapshot entry fields, including Git status, state, SHA-256, and byte count.

In this slice:

- `VS3-GATE-003` - local/dev reports cannot overclaim by tampering with source snapshot entry metadata.
- `VS3-GATE-004` - the native `cornerstone scenario gate ... --json` path validates snapshot entry consistency.
- `VS3-REG-004` - coverage gates detect source snapshot metadata mutation before a release claim.
...
- A tampered local/dev report with a changed source snapshot entry `status` fails the gate.
- The older source-tree validators still pass for that tamper, proving this guard covers a distinct gap.
- A valid local/dev report passes with `entry_mismatches=[]`, `duplicate_paths=[]`, `current_duplicate_paths=[]`, `invalid_entries=[]`, and `current_invalid_entries=[]`.
...
```text
verify_returncode 0
target_path packages/cornerstone_cli/main.py
original_status M
...
The tamper changed only the snapshot entry `status` for `packages/cornerstone_cli/main.py`, while preserving path, state, hash, byte count, source-tree hash, dirty paths, generated dirty paths, command transcripts, and self-command transcript source-tree copies.

That meant the gate could prove the path set was complete and the file bytes matched, but could not prove per-entry source snapshot fields matched the current source snapshot.

## Implementation Evidence

Code and tests:

- `packages/cornerstone_cli/main.py` adds `_vs3_source_tree_snapshot_entry_validation`.
- `packages/cornerstone_cli/main.py` includes `source_tree_snapshot_entry_validation` in the gate payload and source report summary.
- `packages/cornerstone_cli/main.py` fails local/dev assurance reports with `CS_VS3_SOURCE_TREE_SNAPSHOT_ENTRIES_INVALID` when recorded snapshot entry fields differ from the current source snapshot.
- `tests/scenario/test_scaffold_cli.py` asserts the valid gate path keeps snapshot entry validation clean.
- `tests/scenario/test_scaffold_cli.py` adds `test_vs3_scenario_gate_rejects_local_dev_claim_with_tampered_source_snapshot_status`.
...
./cornerstone scenario verify vs3-onprem-trusted-extension --json \
  ...
  > /tmp/cs-vs3-snapshot-entry-verify.json
```
...
./cornerstone scenario gate reports/scenario/vs3-onprem-trusted-extension-2026-06-29.json --json \
  > /tmp/cs-vs3-snapshot-entry-gate.json
```
...
- `blocking_count`: `0`
- `errors`: `[]`
- `source_tree_snapshot_entry_validation.status`: `passed`
- `source_tree_snapshot_entry_validation.checked_path_count`: `16`
- `source_tree_snapshot_entry_validation.entry_mismatches`: `[]`
- `source_tree_snapshot_entry_validation.duplicate_paths`: `[]`
- `source_tree_snapshot_entry_validation.current_duplicate_paths`: `[]`
- `source_tree_snapshot_entry_validation.invalid_entries`: `[]`
- `source_tree_snapshot_entry_validation.current_invalid_entries`: `[]`
- `source_tree_current_validation.status`: `passed`
- `source_tree_snapshot_path_validation.status`: `passed`
- `source_tree_snapshot_coverage_validation.status`: `passed`
- `dirty_path_validation.status`: `passed`
...
```bash
./cornerstone scenario gate reports/scenario/vs3-onprem-trusted-extension-2026-06-29.json --json \
  > /tmp/cs-vs3-snapshot-entry-post-checkpoint-gate.json
...
- post-checkpoint `source_tree_snapshot_entry_validation.status`: `passed`
- post-checkpoint `source_tree_snapshot_entry_validation.checked_path_count`: `16`
- post-checkpoint `source_tree_snapshot_entry_validation.entry_mismatches`: `[]`
- post-checkpoint `scenario_gate_conditions.source_tree_snapshot_entries`: `true`
