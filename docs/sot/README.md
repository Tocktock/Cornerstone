...
| `../verification-reports/VS2_POLICY_TENANCY_EGRESS_IMPLEMENTATION_REPORT_2026-06-19.md` | Final local deterministic VS2 implementation report: 86 AI-verifiable PASS rows and 7 HUMAN_REQUIRED rows |
...
The VS2 policy, tenant isolation, and default egress-deny contract is `../scenario-contracts/VS2_POLICY_TENANCY_EGRESS_CONTRACT.md`. It contains 93 `VS2-SEC-*` rows: 70 MUST_PASS, 16 REGRESSION, and 7 HUMAN_REQUIRED. Local deterministic evidence is recorded in `reports/security/vs2-local-security-proof.json` and `reports/scenario/vs2-policy-tenancy-egress-2026-06-19.json`; final narrative evidence is in `../verification-reports/VS2_POLICY_TENANCY_EGRESS_IMPLEMENTATION_REPORT_2026-06-19.md`. The native verifier is `cornerstone scenario verify vs2-policy-tenancy-egress --json`. The verifier may claim only local VS2 readiness; production security, real IdP, production network, live-provider, human UX, and migration/restore readiness remain blocked by H02-H07 evidence.
