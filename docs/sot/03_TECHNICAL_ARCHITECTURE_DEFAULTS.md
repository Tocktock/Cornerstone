...
| Model access | Local-first: Ollama `ornith:9b` for generation and `qwen3-embedding:0.6b` for embeddings as the default assumption (`ornith:9b` verified installed 2026-07-12); `ornith:35b` only for explicitly named larger-model tests; deterministic `local_test` provider for CI; provider-neutral router direction preserved; external providers optional and named per-scenario when assumed |
