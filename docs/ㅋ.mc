...
client = QdrantClient(":memory:")  # for dev, or QdrantClient(host="localhost", port=6333) for server  
...
This defines a collection named `"embeddings"` with vector size `D` and cosine similarity metric. (Cosine distance is a common choice for text embeddings.) We use `recreate_collection` to drop any existing and start fresh each run (for deterministic prototyping), or `create_collection` if we want to preserve data. If the user switches embedding models (thus dimension changes), recreating ensures the collection schema matches the current model’s vector size. In a more advanced setup, one could maintain separate collections per embedding model or re-index data when switching.

**Data Ingestion (Upserting Vectors)**: With the collection ready, we will need to insert vectors for our data. Depending on the use-case, this could be done by embedding a corpus of documents or receiving user text to store. Assuming we have some text data to index, we convert each piece of text to its embedding (via the chosen model) and then **upsert** it into Qdrant. Using the client, we can batch inserts for efficiency. For example:
...
The `PointStruct` holds an `id`, the embedding vector, and an optional payload (we include the original text or any metadata as payload). Qdrant recommends uploading in batches (e.g. 100 or 1000 at a time) for large datasets. The `upload_points` (or `upsert` method) will add these vectors to the collection. In our prototype, we might embed a small set of example texts for demonstration. After upserting, we can confirm the collection size via `client.count()` if needed. By the end of this step, our Qdrant collection holds all vectors ready for similarity search.
...
results = client.search(collection_name="embeddings", query_vector=query_vec, limit=5)
for hit in results:
...
One advantage of Qdrant is we can run it in-memory for tests and development, avoiding external database setup. When deploying, we can run Qdrant as a separate service (with persistence). It provides REST and gRPC APIs under the hood, but the Python client abstracts those details. The design is also scalable to multi-tenant use (we could have multiple collections or use metadata filters to isolate data per user or per bot, as noted in Qdrant’s docs and our earlier research). Overall, using Qdrant will give us a robust vector store to manage our embeddings and perform similarity lookups quickly, which is essential for any embedding-based search or Q&A system.
...
This will render a list of results if they exist. We keep the design minimal (no complex CSS or JS) to focus on functionality. FastAPI can also serve static files (like a CSS file) if we want to add basic styling, by using `app.mount("/static", StaticFiles(directory="static"), name="static")` and referring to them in the template, but we might skip heavy styling for now.
