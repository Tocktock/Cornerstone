...
3. **Local model stack is the default assumption** for all test and scenario planning: Ollama `ornith:35b` for generation and `qwen3-embedding:0.6b` for embeddings (both verified installed locally, 2026-07-04). External providers (Claude/GPT/Gemini) are optional, future-facing, and must be named per-scenario when assumed. The deterministic `local_test` provider remains the Plane 1 CI baseline.
