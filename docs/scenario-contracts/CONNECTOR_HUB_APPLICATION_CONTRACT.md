...
**Date:** 2026-06-23
**Owner:** JiYong / Tars
**Status:** Frozen task-scoped ConnectorHub adoption contract. PASS and HUMAN_REQUIRED counts belong to generated reports, not this frozen contract header.
**Implementation status:** See `reports/scenario/connector-contract-adapter/aggregate-2026-06-23.json` and `docs/verification-reports/CONNECTOR_HUB_ENGINEERING_TRAIL_INDEX_2026-06-24.md`.
**Scope note:** The frozen scenario set is explicit, with no generic status-neutral backlog rows in this contract.
...
- Declared local supportdesk fixture Action execution and fixture outcome re-ingest for `CS-CH-031`, including WorkflowRun, Action Result, provider receipt, connected outcome, outcome Artifact, and outcome Evidence Bundle records.
...
- Live side-effecting connector Actions. `CS-CH-031` covers only a deterministic local fixture Action and fixture outcome re-ingest; it does not claim live-provider side effects.
...
| Field | Value |
|---|---|
| Trigger / Action | All CS-CH-030 gates pass for a declared local supportdesk fixture ticket-update Action and `cornerstone action execute <action_id> --json` is invoked, then invoked again as an idempotency replay. |
...
| Field | Value |
|---|---|
| Trigger / Action | A declared local supportdesk fixture ticket-update Action executes successfully with an idempotency key; execution is invoked again from a separate CLI process with the same key/request digest; then another approved Action attempts to reuse the same idempotency key for a different target/input digest. |
