...
from cornerstone.api.routes.connectors import (
    ...
    _get_source_or_404,
    _get_source_selection_for_sync,
...
async def create_source_sync_job(
    ...
    selection = _get_source_selection_for_sync(source_id, store)
...
async def retry_sync_job(
    ...
    return await create_source_sync_job(
        prior.datasource_id,
        CreateSyncJobRequest(trigger=SyncJobTrigger.RETRY, created_by=prior.created_by, run_inline=False, max_attempts=prior.max_attempts),
        store,
        settings,
    )
