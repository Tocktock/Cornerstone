...
_RUNTIME_PROVIDERS = {DataSourceType.NOTION}
    ...
    updated_source = data_source.model_copy(
        update={
            ...
            "last_error": None if accessible_count else ErrorInfo(
                ...
                message="No readable Notion pages or databases were found. Share content with the integration and retry discovery.",
            ),
        },
        deep=True,
    )
...
def _add_sync_job_event(
    ...
    return cast(SyncJobEvent, store.add_sync_job_event(
        SyncJobEvent(
            sync_job_id=job.id,
