...
-- Running upgrade 0012_concept_aliases_ontology_graph -> 0013_ontology_extraction_candidates

CREATE TABLE ontology_extraction_runs (
    id UUID NOT NULL, 
    ...
    focus_concept TEXT, 
    evidence_fragment_ids JSONB NOT NULL, 
    artifact_ids JSONB NOT NULL, 
    concept_candidate_count INTEGER DEFAULT '0' NOT NULL, 
    relation_candidate_count INTEGER DEFAULT '0' NOT NULL, 
...
CREATE INDEX ix_ontology_extraction_runs_status ON ontology_extraction_runs (status);

CREATE INDEX ix_ontology_extraction_runs_created_at ON ontology_extraction_runs (created_at);

CREATE INDEX ix_ontology_extraction_runs_requested_by ON ontology_extraction_runs (requested_by);
...
CREATE TABLE concept_candidates (
    ...
    status VARCHAR(32) NOT NULL, 
    matched_existing_concept_id UUID, 
    rationale TEXT, 
    validation_errors JSONB NOT NULL, 
    created_at TIMESTAMP WITH TIME ZONE NOT NULL, 
    PRIMARY KEY (id), 
    CONSTRAINT uq_concept_candidates_run_normalized_name UNIQUE (extraction_run_id, normalized_name), 
    FOREIGN KEY(extraction_run_id) REFERENCES ontology_extraction_runs (id) ON DELETE CASCADE, 
    FOREIGN KEY(matched_existing_concept_id) REFERENCES concepts (id) ON DELETE SET NULL
);

CREATE INDEX ix_concept_candidates_extraction_run_id ON concept_candidates (extraction_run_id);

CREATE INDEX ix_concept_candidates_status ON concept_candidates (status);

CREATE INDEX ix_concept_candidates_normalized_name ON concept_candidates (normalized_name);

CREATE TABLE relation_candidates (
    id UUID NOT NULL, 
    extraction_run_id UUID NOT NULL, 
    source_name CITEXT NOT NULL, 
    target_name CITEXT NOT NULL, 
    normalized_source_name CITEXT NOT NULL, 
    normalized_target_name CITEXT NOT NULL, 
    ...
    created_at TIMESTAMP WITH TIME ZONE NOT NULL, 
    PRIMARY KEY (id), 
    CONSTRAINT ck_relation_candidates_distinct_terms CHECK (normalized_source_name <> normalized_target_name), 
    CONSTRAINT uq_relation_candidates_run_source_target_type UNIQUE (extraction_run_id, normalized_source_name, normalized_target_name, relation_type), 
    FOREIGN KEY(extraction_run_id) REFERENCES ontology_extraction_runs (id) ON DELETE CASCADE, 
    FOREIGN KEY(source_candidate_id) REFERENCES concept_candidates (id) ON DELETE SET NULL, 
    FOREIGN KEY(target_candidate_id) REFERENCES concept_candidates (id) ON DELETE SET NULL, 
    FOREIGN KEY(source_concept_id) REFERENCES concepts (id) ON DELETE SET NULL, 
    FOREIGN KEY(target_concept_id) REFERENCES concepts (id) ON DELETE SET NULL
);

CREATE INDEX ix_relation_candidates_extraction_run_id ON relation_candidates (extraction_run_id);

CREATE INDEX ix_relation_candidates_status ON relation_candidates (status);

CREATE INDEX ix_relation_candidates_relation_type ON relation_candidates (relation_type);

UPDATE alembic_version SET version_num='0013_ontology_extraction_candidates' WHERE alembic_version.version_num = '0012_concept_aliases_ontology_graph';

-- Running upgrade 0013_ontology_extraction_candidates -> 0014_ontology_candidate_review_workflow

ALTER TABLE concept_candidates ADD COLUMN reviewed_by TEXT;

ALTER TABLE concept_candidates ADD COLUMN reviewed_at TIMESTAMP WITH TIME ZONE;

ALTER TABLE concept_candidates ADD COLUMN review_note TEXT;

ALTER TABLE concept_candidates ADD COLUMN promoted_concept_id UUID;

ALTER TABLE concept_candidates ADD COLUMN merged_into_concept_id UUID;

ALTER TABLE concept_candidates ADD CONSTRAINT fk_concept_candidates_promoted_concept_id_concepts FOREIGN KEY(promoted_concept_id) REFERENCES concepts (id) ON DELETE SET NULL;

ALTER TABLE concept_candidates ADD CONSTRAINT fk_concept_candidates_merged_into_concept_id_concepts FOREIGN KEY(merged_into_concept_id) REFERENCES concepts (id) ON DELETE SET NULL;

CREATE INDEX ix_concept_candidates_reviewed_at ON concept_candidates (reviewed_at);

CREATE INDEX ix_concept_candidates_promoted_concept_id ON concept_candidates (promoted_concept_id);

CREATE INDEX ix_concept_candidates_merged_into_concept_id ON concept_candidates (merged_into_concept_id);

ALTER TABLE relation_candidates ADD COLUMN reviewed_by TEXT;

ALTER TABLE relation_candidates ADD COLUMN reviewed_at TIMESTAMP WITH TIME ZONE;

ALTER TABLE relation_candidates ADD COLUMN review_note TEXT;

ALTER TABLE relation_candidates ADD COLUMN promoted_relation_id UUID;

ALTER TABLE relation_candidates ADD COLUMN merged_into_relation_id UUID;

ALTER TABLE relation_candidates ADD CONSTRAINT fk_relation_candidates_promoted_relation_id FOREIGN KEY(promoted_relation_id) REFERENCES concept_relations (id) ON DELETE SET NULL;

ALTER TABLE relation_candidates ADD CONSTRAINT fk_relation_candidates_merged_relation_id FOREIGN KEY(merged_into_relation_id) REFERENCES concept_relations (id) ON DELETE SET NULL;

CREATE INDEX ix_relation_candidates_reviewed_at ON relation_candidates (reviewed_at);

CREATE INDEX ix_relation_candidates_promoted_relation_id ON relation_candidates (promoted_relation_id);

CREATE INDEX ix_relation_candidates_merged_into_relation_id ON relation_candidates (merged_into_relation_id);

UPDATE alembic_version SET version_num='0014_ontology_candidate_review_workflow' WHERE alembic_version.version_num = '0013_ontology_extraction_candidates';

-- Running upgrade 0014_ontology_candidate_review_workflow -> 0015_ontology_graph_evaluation

CREATE TABLE ontology_graph_eval_tasks (
    id UUID NOT NULL, 
    name TEXT NOT NULL, 
    ...
    expected_freshness_state VARCHAR(32), 
    required_concept_ids JSONB DEFAULT '[]' NOT NULL, 
    required_relation_ids JSONB DEFAULT '[]' NOT NULL, 
    required_evidence_fragment_ids JSONB DEFAULT '[]' NOT NULL, 
    require_official_graph BOOLEAN DEFAULT true NOT NULL, 
    require_evidence BOOLEAN DEFAULT true NOT NULL, 
    min_evidence_count INTEGER DEFAULT '1' NOT NULL, 
    min_node_count INTEGER DEFAULT '1' NOT NULL, 
    min_edge_count INTEGER DEFAULT '0' NOT NULL, 
    require_review_provenance BOOLEAN DEFAULT true NOT NULL, 
    max_pending_candidate_count INTEGER, 
    tags JSONB DEFAULT '[]' NOT NULL, 
    created_by TEXT NOT NULL, 
    created_at TIMESTAMP WITH TIME ZONE NOT NULL, 
    updated_at TIMESTAMP WITH TIME ZONE NOT NULL, 
    metadata JSONB DEFAULT '{}' NOT NULL, 
    PRIMARY KEY (id), 
    CONSTRAINT ck_ontology_graph_eval_tasks_depth_v1 CHECK (depth >= 0 AND depth <= 1)
);

CREATE INDEX ix_ontology_graph_eval_tasks_created_at ON ontology_graph_eval_tasks (created_at);

CREATE INDEX ix_ontology_graph_eval_tasks_concept_query ON ontology_graph_eval_tasks (concept_query);

CREATE INDEX ix_ontology_graph_eval_tasks_expected_trust ON ontology_graph_eval_tasks (expected_trust_label);

CREATE TABLE ontology_graph_eval_results (
    id UUID NOT NULL, 
    task_id UUID NOT NULL, 
    response_id UUID NOT NULL, 
    concept_query TEXT NOT NULL, 
    ...
    trust_label VARCHAR(64) NOT NULL, 
    response JSONB NOT NULL, 
    graph_found BOOLEAN NOT NULL, 
    graph_depth_respected BOOLEAN NOT NULL, 
    node_requirements_met BOOLEAN NOT NULL, 
    edge_requirements_met BOOLEAN NOT NULL, 
    evidence_valid BOOLEAN NOT NULL, 
    provenance_present BOOLEAN NOT NULL, 
    trust_label_correct BOOLEAN NOT NULL, 
    freshness_policy_respected BOOLEAN NOT NULL, 
    official_graph_safe BOOLEAN NOT NULL, 
    candidate_boundary_respected BOOLEAN NOT NULL, 
    relation_integrity_valid BOOLEAN NOT NULL, 
    citation_validity_rate FLOAT NOT NULL, 
    success BOOLEAN NOT NULL, 
    failure_reasons JSONB DEFAULT '[]' NOT NULL, 
    evaluated_at TIMESTAMP WITH TIME ZONE NOT NULL, 
    evaluated_by TEXT NOT NULL, 
    PRIMARY KEY (id), 
    CONSTRAINT ck_ontology_graph_eval_results_depth_v1 CHECK (depth >= 0 AND depth <= 1), 
    FOREIGN KEY(task_id) REFERENCES ontology_graph_eval_tasks (id) ON DELETE CASCADE
);

CREATE INDEX ix_ontology_graph_eval_results_task ON ontology_graph_eval_results (task_id);

CREATE INDEX ix_ontology_graph_eval_results_success ON ontology_graph_eval_results (success);

CREATE INDEX ix_ontology_graph_eval_results_evaluated_at ON ontology_graph_eval_results (evaluated_at);

CREATE INDEX ix_ontology_graph_eval_results_trust ON ontology_graph_eval_results (trust_label);

UPDATE alembic_version SET version_num='0015_ontology_graph_evaluation' WHERE alembic_version.version_num = '0014_ontology_candidate_review_workflow';

-- Running upgrade 0015_ontology_graph_evaluation -> 0016_ontology_reextraction_runs

ALTER TABLE sync_jobs ADD COLUMN queue_ontology_reextraction BOOLEAN DEFAULT true NOT NULL;

ALTER TABLE sync_jobs ADD COLUMN run_ontology_reextraction_inline BOOLEAN DEFAULT false NOT NULL;

ALTER TABLE sync_jobs ADD COLUMN ontology_focus_concept TEXT;

CREATE TABLE ontology_reextraction_runs (
    id UUID NOT NULL, 
    datasource_id UUID NOT NULL, 
    provider VARCHAR(32) NOT NULL, 
    ...
    focus_concept TEXT, 
    reason TEXT, 
    source_external_ids JSONB DEFAULT '[]' NOT NULL, 
    artifact_ids JSONB DEFAULT '[]' NOT NULL, 
    changed_artifact_ids JSONB DEFAULT '[]' NOT NULL, 
    evidence_fragment_ids JSONB DEFAULT '[]' NOT NULL, 
    extraction_run_ids JSONB DEFAULT '[]' NOT NULL, 
    concept_candidate_count INTEGER DEFAULT '0' NOT NULL, 
    relation_candidate_count INTEGER DEFAULT '0' NOT NULL, 
    warning_count INTEGER DEFAULT '0' NOT NULL, 
    official_graph_mutated BOOLEAN DEFAULT false NOT NULL, 
    error TEXT, 
    created_at TIMESTAMP WITH TIME ZONE NOT NULL, 
    started_at TIMESTAMP WITH TIME ZONE, 
    completed_at TIMESTAMP WITH TIME ZONE, 
...
CREATE INDEX ix_ontology_reextraction_runs_datasource_status ON ontology_reextraction_runs (datasource_id, status);

CREATE INDEX ix_ontology_reextraction_runs_sync_job ON ontology_reextraction_runs (sync_job_id);

CREATE INDEX ix_ontology_reextraction_runs_created_at ON ontology_reextraction_runs (created_at);

CREATE INDEX ix_ontology_reextraction_runs_trigger ON ontology_reextraction_runs (trigger);

UPDATE alembic_version SET version_num='0016_ontology_reextraction_runs' WHERE alembic_version.version_num = '0015_ontology_graph_evaluation';
