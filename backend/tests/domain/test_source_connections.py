...
def test_connector_permission_checks_cover_manager_and_action_grants():
    with _session() as session:
        ...
        session.add_all([manager, delegated, admin])
        session.add(
            ConnectorScopeGrant(
...
def test_run_sync_marks_deleted_notion_entries_monitoring_without_degrading(tmp_path: Path):
    ...
    with _session() as session:
        ...
        first_result = run_sync(session, connection, settings, trigger_kind="initial")
        assert first_result.artifact_count == 2

        scopes["databases"][scope_id]["entries"] = scopes["databases"][scope_id]["entries"][:1]
        (fixture_root / "scopes.json").write_text(json.dumps(scopes), encoding="utf-8")

        second_result = run_sync(session, connection, settings, trigger_kind="scheduled")
        session.flush()
