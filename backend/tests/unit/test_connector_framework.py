...
from cornerstone.connectors.google_drive import GoogleDriveConnector
...
def test_google_drive_connector_catalog_is_available() -> None:
    definitions = list_connector_definitions(Settings())
    google_drive = next(definition for definition in definitions if definition.provider == DataSourceType.GOOGLE_DRIVE)

    assert google_drive.display_name == "Google Drive"
    assert google_drive.availability == "available"
    assert google_drive.auth_type == "oauth2"
    assert google_drive.required_scopes[0].key == "https://www.googleapis.com/auth/drive.readonly"
    assert google_drive.discoverable_objects == ["document", "spreadsheet", "presentation", "pdf", "folder", "file"]
    assert google_drive.ingestible_objects == ["document", "text_file"]


def test_google_drive_authorization_url_contains_required_oauth_state() -> None:
    settings = Settings()
    ...
    intent = ConnectionIntent(
        ...
        redirect_uri="http://localhost:8000/v1/oauth/google_drive/callback",
        state_nonce="state-nonce-with-good-length",
        expires_at=utc_now() + timedelta(minutes=15),
    )
    ...
    assert query["client_id"] == [settings.google_drive_client_id]
    assert query["redirect_uri"] == [intent.redirect_uri]
    assert query["response_type"] == ["code"]
...
def test_mock_google_drive_discovery_and_ingestion_flow() -> None:
    settings = Settings()
    connector = GoogleDriveConnector(settings)
    credential = ConnectorCredential(
        datasource_id="source-google-drive",
        provider=DataSourceType.GOOGLE_DRIVE,
    ...
    syncable = [item for item in discovery.objects if item.ingestion_supported]
    assert len(syncable) == 1
    assert syncable[0].object_type == ProviderObjectType.DOCUMENT
    ...
    objects = asyncio.run(
        connector.list_objects(
            ...
            access_token="mock-token",
            selection=None,
            selected_objects=syncable,
        )
    )

    assert len(objects) == 1
    assert objects[0].source_external_id == "google-doc-1"
    assert objects[0].source_object_type == "document"
    assert "shared organizational context layer" in objects[0].content
