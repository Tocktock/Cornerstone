...
async def run_sync_job_once(
    *,
    ...
    selection = get_source_selection_for_sync(claimed.datasource_id, store)
    ...
    with store.transaction():
        ...
        _add_sync_job_event(
            ...
            "Sync job succeeded.",
            {
                ...
                "artifactChangedCount": response.artifact_changed_count,
                "evidenceCreatedCount": saved.evidence_created_count,
            },
        )
        _queue_ontology_reextraction_after_sync(
            store=store,
            job=saved,
            response=response,
        )
...
def _queue_ontology_reextraction_after_sync(*, store: Any, job: SyncJob, response: Any) -> None:
    if not job.queue_ontology_reextraction:
        return
    trigger = (
        OntologyReExtractionTrigger.SCHEDULED_SYNC
        if job.trigger == SyncJobTrigger.SCHEDULED
        else OntologyReExtractionTrigger.CONNECTOR_SYNC
    )
    reextraction = OntologyReExtractionService(store).queue_from_sync_response(
        response=response,
        trigger=trigger,
        created_by=job.created_by,
        sync_job_id=job.id,
        focus_concept=job.ontology_focus_concept,
    )
    if reextraction is None:
        _add_sync_job_event(
            store,
            job,
            "ontology.reextraction_skipped",
            "Ontology re-extraction was skipped because sync reused existing Artifacts.",
            {"artifactCreatedCount": response.artifact_created_count},
        )
        return
    if job.run_ontology_reextraction_inline:
        reextraction_response = OntologyReExtractionService(store).run(
            reextraction.id,
            RunOntologyReExtractionRunRequest(requested_by=job.created_by),
        )
        reextraction = reextraction_response.run
    _add_sync_job_event(
        store,
        job,
        "ontology.reextraction_queued",
        "Ontology re-extraction was queued from changed source artifacts; output remains candidate-only.",
        {
            "reExtractionRunId": reextraction.id,
            "status": str(reextraction.status),
            "artifactIds": list(reextraction.artifact_ids),
            "changedArtifactIds": list(reextraction.changed_artifact_ids),
            "evidenceFragmentIds": list(reextraction.evidence_fragment_ids),
            "officialGraphMutated": reextraction.official_graph_mutated,
        },
    )
