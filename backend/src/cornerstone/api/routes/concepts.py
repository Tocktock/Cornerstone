...
def create_concept(
    ...
    _validate_concept_terms_available(request.name, request.aliases, store)
    concept = Concept(
        name=request.name,
...
def _validate_concept_terms_available(name: str, aliases: list[str], store: InMemoryStore) -> None:
    ...
    for concept in store.list_concepts():
        ...
        if overlap:
            ...
            log_event(
                "concept.create_blocked",
                reason="concept_term_already_exists",
                conceptId=concept.id,
                conceptName=concept.name,
                term=blocked_term,
            )
            raise HTTPException(
                ...
                detail=f"Concept name or alias already exists: {blocked_term}",
            )
