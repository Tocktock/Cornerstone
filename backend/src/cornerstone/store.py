...
class _Snapshot(TypedDict):
    ...
    ontology_extraction_runs: dict[str, OntologyExtractionRun]
    ...
    ontology_graph_eval_tasks: dict[str, OntologyGraphEvalTask]
    ontology_graph_eval_results: dict[str, OntologyGraphEvalResult]
    ontology_reextraction_runs: dict[str, OntologyReExtractionRun]
...
class InMemoryStore:
    ...
    def __init__(self) -> None:
        ...
        self._ontology_extraction_runs: dict[str, OntologyExtractionRun] = {}
        self._concept_candidates: dict[str, ConceptCandidate] = {}
        self._relation_candidates: dict[str, RelationCandidate] = {}
        ...
        self._ontology_graph_eval_tasks: dict[str, OntologyGraphEvalTask] = {}
        self._ontology_graph_eval_results: dict[str, OntologyGraphEvalResult] = {}
        self._ontology_reextraction_runs: dict[str, OntologyReExtractionRun] = {}
    ...
    def reset(self) -> None:
        with self._lock:
            ...
            self._ontology_extraction_runs.clear()
            self._concept_candidates.clear()
            self._relation_candidates.clear()
            ...
            self._ontology_graph_eval_tasks.clear()
            self._ontology_graph_eval_results.clear()
            self._ontology_reextraction_runs.clear()
    ...
    def add_ontology_extraction_run(self, run: OntologyExtractionRun) -> OntologyExtractionRun:
        with self._lock:
            self._ontology_extraction_runs[run.id] = run.model_copy(deep=True)
            return run.model_copy(deep=True)

    def update_ontology_extraction_run(self, run: OntologyExtractionRun) -> OntologyExtractionRun:
        with self._lock:
            if run.id not in self._ontology_extraction_runs:
    ...
    def get_ontology_extraction_run(self, run_id: str) -> OntologyExtractionRun:
        with self._lock:
            run = self._ontology_extraction_runs.get(run_id)
            if run is None:
    ...
    def list_ontology_extraction_runs(self) -> list[OntologyExtractionRun]:
        with self._lock:
            return [item.model_copy(deep=True) for item in self._ontology_extraction_runs.values()]

    def add_concept_candidate(self, candidate: ConceptCandidate) -> ConceptCandidate:
        with self._lock:
            self._concept_candidates[candidate.id] = candidate.model_copy(deep=True)
            return candidate.model_copy(deep=True)

    def get_concept_candidate(self, candidate_id: str) -> ConceptCandidate:
        with self._lock:
            candidate = self._concept_candidates.get(candidate_id)
    ...
    def update_concept_candidate(self, candidate: ConceptCandidate) -> ConceptCandidate:
        with self._lock:
            if candidate.id not in self._concept_candidates:
    ...
    def list_concept_candidates(
        ...
        *,
        ...
        with self._lock:
            candidates = list(self._concept_candidates.values())
            if extraction_run_id is not None:
                candidates = [item for item in candidates if item.extraction_run_id == extraction_run_id]
    ...
    def add_relation_candidate(self, candidate: RelationCandidate) -> RelationCandidate:
        with self._lock:
            self._relation_candidates[candidate.id] = candidate.model_copy(deep=True)
            return candidate.model_copy(deep=True)

    def get_relation_candidate(self, candidate_id: str) -> RelationCandidate:
        with self._lock:
            candidate = self._relation_candidates.get(candidate_id)
            if candidate is None:
    ...
    def update_relation_candidate(self, candidate: RelationCandidate) -> RelationCandidate:
        with self._lock:
            if candidate.id not in self._relation_candidates:
    ...
    def list_relation_candidates(
        ...
        *,
        ...
        with self._lock:
            candidates = list(self._relation_candidates.values())
            if extraction_run_id is not None:
                candidates = [item for item in candidates if item.extraction_run_id == extraction_run_id]
            if status is not None:
                candidates = [item for item in candidates if item.status == status]
    ...
    def add_ontology_reextraction_run(self, run: OntologyReExtractionRun) -> OntologyReExtractionRun:
        with self._lock:
            self._ontology_reextraction_runs[run.id] = run.model_copy(deep=True)
            return run.model_copy(deep=True)

    def update_ontology_reextraction_run(self, run: OntologyReExtractionRun) -> OntologyReExtractionRun:
        with self._lock:
            if run.id not in self._ontology_reextraction_runs:
    ...
    def get_ontology_reextraction_run(self, run_id: str) -> OntologyReExtractionRun:
        with self._lock:
            run = self._ontology_reextraction_runs.get(run_id)
            if run is None:
    ...
    def list_ontology_reextraction_runs(
        ...
        *,
        ...
        with self._lock:
            runs = list(self._ontology_reextraction_runs.values())
            if datasource_id is not None:
                runs = [item for item in runs if item.datasource_id == datasource_id]
            if status is not None:
                runs = [item for item in runs if item.status == status]
            runs = sorted(runs, key=lambda item: (item.created_at, item.id))
    ...
    def add_ontology_graph_eval_task(self, task: OntologyGraphEvalTask) -> OntologyGraphEvalTask:
        with self._lock:
            self._ontology_graph_eval_tasks[task.id] = task.model_copy(deep=True)
            return task.model_copy(deep=True)

    def get_ontology_graph_eval_task(self, task_id: str) -> OntologyGraphEvalTask:
        with self._lock:
            task = self._ontology_graph_eval_tasks.get(task_id)
            if task is None:
    ...
    def list_ontology_graph_eval_tasks(self) -> list[OntologyGraphEvalTask]:
        with self._lock:
            tasks = sorted(self._ontology_graph_eval_tasks.values(), key=lambda item: (item.created_at, item.id))
            return [item.model_copy(deep=True) for item in tasks]

    def add_ontology_graph_eval_result(self, result: OntologyGraphEvalResult) -> OntologyGraphEvalResult:
        with self._lock:
            if result.task_id not in self._ontology_graph_eval_tasks:
                raise NotFoundError(f"OntologyGraphEvalTask not found: {result.task_id}")
            self._ontology_graph_eval_results[result.id] = result.model_copy(deep=True)
            return result.model_copy(deep=True)

    def get_ontology_graph_eval_result(self, result_id: str) -> OntologyGraphEvalResult:
        with self._lock:
            result = self._ontology_graph_eval_results.get(result_id)
            if result is None:
    ...
    def list_ontology_graph_eval_results(self, task_id: str | None = None) -> list[OntologyGraphEvalResult]:
        with self._lock:
            results = list(self._ontology_graph_eval_results.values())
            if task_id is not None:
                results = [item for item in results if item.task_id == task_id]
            results.sort(key=lambda item: (item.evaluated_at, item.id))
            return [item.model_copy(deep=True) for item in results]

    # Backward-compatible grounded-context evaluation aliases
    def add_eval_task(self, task: GroundedContextEvalTask) -> GroundedContextEvalTask:
        return self.add_grounded_context_eval_task(task)

    def get_eval_task(self, task_id: str) -> GroundedContextEvalTask:
        return self.get_grounded_context_eval_task(task_id)

    def list_eval_tasks(self) -> list[GroundedContextEvalTask]:
        return self.list_grounded_context_eval_tasks()

    def add_eval_result(self, result: GroundedContextEvalResult) -> GroundedContextEvalResult:
        return self.add_grounded_context_eval_result(result)

    def get_eval_result(self, result_id: str) -> GroundedContextEvalResult:
        return self.get_grounded_context_eval_result(result_id)

    def list_eval_results(self, task_id: str | None = None) -> list[GroundedContextEvalResult]:
        return self.list_grounded_context_eval_results(task_id=task_id)
    ...
    def _snapshot_unlocked(self) -> _Snapshot:
        return {
            ...
            "ontology_extraction_runs": copy.deepcopy(self._ontology_extraction_runs),
            "concept_candidates": copy.deepcopy(self._concept_candidates),
            "relation_candidates": copy.deepcopy(self._relation_candidates),
            ...
            "ontology_graph_eval_tasks": copy.deepcopy(self._ontology_graph_eval_tasks),
            "ontology_graph_eval_results": copy.deepcopy(self._ontology_graph_eval_results),
            "ontology_reextraction_runs": copy.deepcopy(self._ontology_reextraction_runs),
        }
    ...
    def _restore_unlocked(self, snapshot: _Snapshot) -> None:
        ...
        self._ontology_extraction_runs = copy.deepcopy(snapshot["ontology_extraction_runs"])
        self._concept_candidates = copy.deepcopy(snapshot["concept_candidates"])
        self._relation_candidates = copy.deepcopy(snapshot["relation_candidates"])
        ...
        self._ontology_graph_eval_tasks = copy.deepcopy(snapshot["ontology_graph_eval_tasks"])
        self._ontology_graph_eval_results = copy.deepcopy(snapshot["ontology_graph_eval_results"])
        self._ontology_reextraction_runs = copy.deepcopy(snapshot["ontology_reextraction_runs"])
