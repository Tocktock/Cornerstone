...
def list_connector_definitions(settings: Settings) -> list[ConnectorDefinition]:
    return [
        ...
        ConnectorDefinition(
            ...
            description="Connect selected Google Drive files and ingest Google Docs/text files as organizational evidence sources.",
            auth_type=ConnectorAuthType.OAUTH2,
            availability=ConnectorAvailability.AVAILABLE,
            production_ready=False,
            required_scopes=[
                ConnectorScope(
                    ...
                    description="Read metadata and export selected Google Drive documents shared with the authorized account.",
                )
            ],
            optional_scopes=[
                ConnectorScope(
                    ...
                    description="Reserved for metadata-only discovery flows.",
                    required=False,
                )
            ],
            supported_objects=["document", "text_file"],
            discoverable_objects=["document", "spreadsheet", "presentation", "pdf", "folder", "file"],
            ingestible_objects=["document", "text_file"],
            setup_steps=[
                ConnectorSetupStep(
                    key="authorize",
                    title="Authorize Google Drive",
                    description="Open Google OAuth and grant read-only Drive access.",
                    order=1,
                ),
                ConnectorSetupStep(
                    key="test_connection",
                    title="Test connection",
                    description="Verify that Cornerstone can read Google Drive file metadata.",
                    order=2,
                ),
                ConnectorSetupStep(
                    key="select_sources",
                    title="Select files",
                    description="Choose supported Google Docs or text files to sync before ingestion.",
                    order=3,
                ),
            ],
            limitations=[
                "v1.2.0 ingests selected Google Docs by exporting them as text/plain.",
                "Plain text files can be downloaded and ingested.",
                "Sheets, Slides, PDFs, and folders are discoverable but not selectable for ingestion yet.",
            ],
            ...
            capabilities=ConnectorCapabilities(
                ...
                supports_source_updated_at=True,
                supports_source_urls=True,
                supports_author_metadata=False,
                supports_permission_snapshots=False,
            ),
        ),
        ConnectorDefinition(
